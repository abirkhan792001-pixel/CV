...
check(any("linkedin.com/in/" in (l.get("uri") or "") for l in page.get_links()),
      "LinkedIn profile reachable as a link",
      next((l["uri"] for l in page.get_links() if "linkedin" in (l.get("uri") or "")), "absent"))
...
for h in ("EDUCATION", "WORK EXPERIENCE", "EXTRACURRICULAR", "ADDITIONAL INFORMATION"):
...
pairs = [
    ("Nova School",         "Ranked top 10% of the class"),
    ("Shiv Nadar",          "Graduated Magna Cum Laude"),
    ("SQRlane",             "Designed for EU-hosted open-weight inference"),
    ("SCAILE Technologies", "Owned North American clients"),
    ("Alvarez & Marsal",    "Prepared creditor-negotiation"),
    ("Biome Venture Studio","Developed commercial due diligence"),
    ("Hack-Nation",         "Coordinated the TUM side"),
    ("Impact Consulting",   "Created competitive benchmarking"),
    ("United Nations",      "Led donation drives"),
]
...
check(all(l.lstrip().startswith("•") for l in lines if "Ranked top 10% of the class" in l),
...
# The rules stop at 191.82 mm; italic side-bearing carries the right-aligned
# locations and dates a fraction past them, so the rightmost ink on the page is
# 191.96 mm. That is the ceiling nothing may cross.
EDGE = 191.96
rights = [round(b[2] * PT, 3) for b, t in spans]
check(max(rights) <= EDGE + 0.01, "nothing crosses the text right edge",
      f"widest line ends at {max(rights):.2f} mm")
flush = [r for r in rights if r > EDGE - 0.10]
# 9 entries x 2 rows (organisation/location, then role/dates). An exact count
# rather than a floor, so a silently dropped entry fails here too. The spread is
